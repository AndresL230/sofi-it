...
| 7 | API key absent from client bundle (`grep` the dist output) | see deploy report | Filled in by the deploy step |
| 8 | Rate limit returns 429 under hammering | see deploy report | Filled in by the deploy step (KV counter per IP, 20/hour → 429 + `retry-after: 3600`) |
...
Bundle (from `dist/` after `npm run build`, gzip): index 87 KB · vendor 53 KB · motion 38 KB · charts 104 KB · SparkAreaChart 0.5 KB · CSS 7 KB ≈ 289 KB total. Under the ~350 KB target — PASS.

Caveat on the charts chunk: `price_creep.tsx` loads `SparkAreaChart` through `React.lazy`, and `SparkAreaChart` is the only module that imports `recharts`. But in the current `dist/` the entry chunk still carries a static `import{c as …}from"./charts-…js"` (a small shared dependency Rollup placed in the `charts` manual chunk) and `dist/index.html` emits `<link rel="modulepreload">` for it, so the 104 KB chunk is fetched on first load rather than deferred. The lazy split is in place; the manual-chunk assignment in `vite.config.ts` undoes its benefit. Not an acceptance failure (total is under target) — noted for follow-up.
