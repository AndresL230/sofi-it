...
- workers.dev URL: https://sofi-purchase-coach.andreslopez-23061.workers.dev — **currently 404 (`error code: 1042`)**: adding `routes` without `workers_dev: true` makes wrangler disable the workers.dev route by default (it warns on deploy). Add `"workers_dev": true` to `wrangler.jsonc` and redeploy if that URL should keep working.
