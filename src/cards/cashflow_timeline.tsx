...
function CashflowTimeline({ today, paydays, inFull, accelerated, redirectMonthly, paycheck }: Props) {
  ...
  return (
    <CardShell className="px-6 pb-[30px] pt-[22px]">
      <div className="relative mx-2 mb-11 mt-9 h-2 rounded-pill bg-lavender-soft">
        ...
        {paydays.length ? <div className="absolute top-4 whitespace-nowrap text-[10.5px] text-slate-muted" style={{ left: x(paydays[Math.min(1, paydays.length - 1)]) }}>paydays · biweekly <Money value={paycheck} size="inline" cents="never" animated={false} /></div> : null}
        ...
        <div className="absolute -top-8 whitespace-nowrap text-[11.5px] font-bold text-purple" style={{ left: `${accX}%`, transform: 'translateX(-40%)' }}><DateText date={accelerated} /> — with <Money value={redirectMonthly} size="inline" cents="never" animated={false} />/mo redirect</div>
...
export default defineCard<Props>({ type: 'cashflow_timeline', section: 'Money context', label: '', condition, select, Component: CashflowTimeline, samples: [{ query: '$1,200 flight to Lisbon in March' }] })
